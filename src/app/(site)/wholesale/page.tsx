"use client";

import { useState } from "react";

const field =
  "mt-1 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-foreground outline-none ring-accent/30 focus:ring-2";

export default function WholesalePage() {
  const [sent, setSent] = useState(false);

  return (
    <div className="mx-auto max-w-xl px-4 py-14 sm:px-6 sm:py-20">
      <h1 className="font-[family-name:var(--font-poppins)] text-3xl font-bold text-foreground">
        Wholesale enquiry
      </h1>
      <p className="mt-3 text-sm text-muted">
        Tell us about your store or brand — MOQ, city, and categories you are
        interested in. Our team will respond within 1–2 business days.
      </p>

      {sent ? (
        <p className="mt-10 rounded-xl border border-brand-red/30 bg-red-50 p-6 text-sm text-foreground">
          Enquiry recorded for demo. Pipe this form to your CRM or email in
          production.
        </p>
      ) : (
        <form
          className="mt-10 space-y-4"
          onSubmit={(e) => {
            e.preventDefault();
            setSent(true);
          }}
        >
          <label className="block">
            <span className="text-xs text-muted">Business / store name</span>
            <input required className={field} />
          </label>
          <label className="block">
            <span className="text-xs text-muted">Contact name</span>
            <input required className={field} />
          </label>
          <label className="block">
            <span className="text-xs text-muted">Email</span>
            <input required type="email" className={field} />
          </label>
          <label className="block">
            <span className="text-xs text-muted">City & state</span>
            <input required className={field} />
          </label>
          <label className="block">
            <span className="text-xs text-muted">What are you looking for?</span>
            <textarea
              required
              rows={4}
              placeholder="Categories, estimated volumes, timeline…"
              className={field}
            />
          </label>
          <button
            type="submit"
            className="w-full rounded-full bg-brand-red py-3 text-sm font-semibold text-white hover:bg-red-700"
          >
            Submit enquiry
          </button>
        </form>
      )}
    </div>
  );
}
